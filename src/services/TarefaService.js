...
class TarefaService {
  ...
  async cadastrar(dados) {
    ...
    if (erros.length) {
      ...
      const dataConclusao = dados.dataConclusao // verifica se a data de conclusao foi informada
  ...
  async editar(id, dados) {
    ...
    if (!id) {
      ...
      // Se a tarefa não existir no banco o pertencer a outro usuário, informamos que ela não existe
      ...
      if (dados.nome && dados.nome.trim() && dados.nome.trim().length < 4) {
        erros.push('Nome da tsrefa precisa ter pelo menos 4 caracteres');
      }
    }
    ...
    if (dados.dataPrevistaConclusao && dados.dataPrevistaConclusao.trim()) {
      dadosAtualizar.dataPrevistaConclusao = new Date(
        dados.dataPrevistaConclusao
      );
    }
